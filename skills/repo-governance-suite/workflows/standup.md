...
- **By recency** (common) — worktrees active in a window:
  ```bash
  node "${CLAUDE_SKILL_DIR}/assets/standup/standup.mjs" worktrees --since <1h|4h|24h|7d|all> --json
  ```
...
- **By hand** — specific branches and/or open PRs:
  ```bash
  node "${CLAUDE_SKILL_DIR}/assets/standup/standup.mjs" worktrees --json   # local branches
  node "${CLAUDE_SKILL_DIR}/assets/standup/standup.mjs" prs --json         # open PRs (via gh)
  ```
...
```bash
node "${CLAUDE_SKILL_DIR}/assets/standup/standup.mjs" open --force --agent facilitator \
...
> `<skill-dir>/assets/standup/agent-brief.md` and play your part by it. The room is
> `~/.claude-mem/STANDUP.md`; speak with `node "<skill-dir>/assets/standup/standup.mjs" post …`,
...
```bash
node "${CLAUDE_SKILL_DIR}/assets/standup/standup.mjs" summation --agent facilitator \
...
```bash
node "${CLAUDE_SKILL_DIR}/assets/standup/standup.mjs" <command> [--flags]
```
