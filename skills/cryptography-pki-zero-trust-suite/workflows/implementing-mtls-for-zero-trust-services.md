...
Use to design, configure, or audit mutual TLS for authorized services, including trust domains, certificate profiles, issuance, rotation, revocation, identity mapping, policy enforcement, observability, and failure handling. Verify platform and TLS-library behavior at runtime. Do not deploy trust changes without staged testing and rollback.
...
_Source workflow alias: implementing-mtls-for-zero-trust-services. Condensed from the validated Codex source for the ChatGPT Web edition; executable examples and local-only artifacts were intentionally omitted._