...
- `remove-ai-marks` requires its backend service running separately (not included in this
  backup — see the skill's SKILL.md for the service repo). The skill will not function
  without it.
- `vibe-coding-security-review` was built from two upstream sources under different licenses
  (MIT structure, GPL-3.0 gap-fill content) — see the skill's SKILL.md provenance section.
